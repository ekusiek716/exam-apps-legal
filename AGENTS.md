...
- 公開対象は `index.md` / `support.md` / `privacy.md` / `terms.md` / `tokushoho.md`。`drafts/` は弁護士レビュー前提の原文ドラフトであり、公開ページと同一とはみなさない。
...
- `git diff --check`
- `node scripts/check-published-drift.mjs` — 本文が公開ページへ実際に反映されているかを実URLで確認する（GitHub Pages のビルドは非同期なので、マージしただけでは届かないことがある）
