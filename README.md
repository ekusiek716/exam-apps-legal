...
公開ページを増やしたら `scripts/check-published-drift.mjs` の `PAGES` にも足す。足し忘れると
そのページだけ検査されないため、直下の markdown と `PAGES` がずれていたら検査自体が落ちる。

検査しないもの: 12文字未満の短い断片（表の「利用中」のような一語は、どのページにも偶然
含まれてしまい検査にならないため）と、`{% for %}` ループの内側。
